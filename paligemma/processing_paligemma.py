# Image2Tensor, tokenizer text prompts by language model (given in `modeling_gemma`) 
...
def process_images(
    ...
    # Move the channel dimension to the first dimension. Expected shape: [num_channels, height, width]
...
class PaliGemmaProcessor:
    ...
    def __init__(
        ...
        # We will add the BOS and EOS tokens overselves 
    ...
    def __call__(
        ...
        assert len(images) == 1 and len(text) == 1, f"Received {len(images)} images and {len(text)} texts."
        ...
        # [batch_size, num_channels, height, width], then convert into a Pytorch tensor
